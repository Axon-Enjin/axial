...
|------|--------|-------|-------|
| Real payer KYB | 🟡 | Eng + vendor | `AXIAL_KYB_MODE=manual\|mock\|vendor`; manual review UI shipped |
| BIR Permit to Transmit + live EIS | ⬜ | Regulatory | `BIR_EIS_LIVE=true`; vault JWS keys — **seam ready** |
| NoA legal text counsel review | ⬜ | Legal | [`clr-axial.md`](clr-axial.md); payer portal uses draft |
| Terms of Use + Privacy Policy | 🟡 | Legal | Trust & Boundary screen with draft watermark |
| Licensed financing partner (RA 8556) | ⬜ | Business | Axial = tech + origination rails |
| Mainnet settle dry-run signed off | 🟡 | Eng | [`settle-dry-run-checklist.md`](settle-dry-run-checklist.md) + [`s0-6-demo-dry-run.md`](s0-6-demo-dry-run.md) |
...
| Per-org TIN config (not demo defaults) | ✅ | Eng | Settings EIS tax profile + `resolveEisPartyDefaults` |
| Custody roadmap (Freighter / MPC) | 🟡 | Eng | Freighter path exists for MSME/payer |
| Monitoring on stuck EIS submissions | ✅ | Eng | `GET /api/eis/monitor` |

## P2 — Gated / partner-dependent

| Gate | Status | Owner | Notes |
|------|--------|-------|-------|
| PDAX Connect API | ❌ | Business | Sandbox not granted; mock UI only — **excluded** |
| Funder marketplace | ⬜ | Product | After single-funder settle solid |
| Funder role RLS (`org_memberships.role=funder`) | ⬜ | Eng | Implement when external LPs need session auth beyond magic token |
...
| 2026-07-15 | Phase 4 seams: KYB modes, Trust & Boundary, EIS monitor, org TIN, funder RLS tracked |
