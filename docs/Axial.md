...
| Operating network — Mainnet default | ✅ Locked (2026-05-22; sandbox toggle 2026-07-18) | **Production operating target is Stellar Mainnet** — all 4 contracts deployed + initialized (`deployments/mainnet.json`). Settings may switch a **browser session** to Testnet sandbox via cookie when `TESTNET_*` / `deployments/testnet.json` are configured. Default remains Mainnet. |
