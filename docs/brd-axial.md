...
Axial is a **liquidity and compliance engine** for Philippine MSMEs. The buyer job is **working capital financing**: give B2B businesses instant cash from confirmed accounts receivable by tokenizing them on Stellar/Soroban and executing atomic swaps into USDC (PHP-denominated at the UX layer) — eliminating the Net 60–90 payroll cash crunch. At the same moment, it handles the regulatory side: automatically splitting and routing statutory payroll obligations (SSS, PhilHealth, Pag-IBIG) and generating and signing BIR Electronic Invoicing System (EIS) payloads, then surfacing them for one-click review and submission within the mandatory T+3 window.

The business case is "Instant Capital, Effortless Compliance." Lead with working capital; keep the dual claim as the product promise — Axial is not another accounting tool that records what happened, and it is not factoring-only. It is infrastructure where liquidity and compliance are the same event, and the founder approves outcomes in one click. *(Tagline + Co-Pilot model locked 2026-06-18; buyer-frame / category stack locked 2026-07-18 — see [Axial.md](Axial.md); auto-submission gated on BIR Permit to Transmit.)*
